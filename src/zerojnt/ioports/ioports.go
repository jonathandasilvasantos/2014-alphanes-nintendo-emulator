...
type PPU_STATUS struct {
	...
	SPRITE_OVERFLOW   bool  // More then 8 sprites in a scanline
	SPRITE_0_BIT      bool  // Set when a nonzero pixel of sprite 0 overlaps a nonzero background pixel; cleared at dot 1 of the pre-render line. Used for raster timing.
...
func RMPPU(IO *IOPorts, cart *cartridge.Cartridge, addr uint16) byte {
	...
	case 0x2007:
		newaddr := cart.Mapper.MapPPU(IO.VRAM_ADDRESS)
		return READ_PPUDATA(IO, cart, newaddr)
	}
	return 0
}
...
func WMPPU(IO *IOPorts, cart *cartridge.Cartridge, addr uint16, value byte) {
	...
	switch addr {
	case 0x4014:
		// This transaction takes ~513 CPU Cycles
	...
	case 0x2007:
		newaddr := cart.Mapper.MapPPU(IO.VRAM_ADDRESS)
		WRITE_PPUDATA(IO, cart, value, newaddr)
	}
}
