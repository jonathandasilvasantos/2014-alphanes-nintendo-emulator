...
func WRITE_PPUDATA(IO *IOPorts, cart *cartridge.Cartridge, value byte, newaddr uint16) {
	IO.PPU_RAM[newaddr] = value
...
func WRITE_OAMDMA(IO *IOPorts, cart *cartridge.Cartridge, value byte) {
	for i := 0; i < 256; i++ {
		...
		prgrom, finaladdr := cart.Mapper.MapCPU(cpuaddr + uint16(i))
		var data byte
		if prgrom {
